<?php 
define("DOMINIO","http://localhost/Anuncios/");
require_once("../../BD/Connection.php");
require_once("../../Models/SingleAnuncio.php");
session_start();
if(empty($_SESSION)){
$ip = "181.176.103.29";
...
if(empty($_GET['id'])){
	...
	$content=Anuncios::SingleAnuncio($_GET['id']);
	if($content==""){
		header("Location:../Buscador/");
	}else{
		$db=Db::getConnect();
		$select=$db->prepare('SELECT direccion,latitud,longitud,categoria,nombre,descripcion,imagenes from tblanuncios where id_anuncio=:id_anuncio  LIMIT 1');
		$select->bindValue('id_anuncio',$_GET['id']);
		$select->execute();
		...
		$categorias=$row['3'];
		$titulo=$row['4'];
		$descripcion=substr($row['5'],0,150);
        $desire=explode(",",$row['6']);
		$relacionados=Anuncios::publicacionesAsociadas($categorias,$_SESSION['pais'],$_GET['id']);
		$populares=Anuncios::MasPopulares($categorias,$_SESSION['pais'],$_GET['id']);
		$likesAds=Anuncios::AnunciosLike($categorias,$_SESSION['pais'],$_GET['id']);
	}
...
<html lang="es" dir="ltr">
	<head>
		...
		<meta property="og:image" content="<?=DOMINIO.$desire[0]?>"/>
		<meta property="og:description" content="<?=$descripcion?>">
		<meta property="og:keywords" content="Kinesiólogas,Anuncios Eroticos,xxx,Mujeres Bonitas,Putas,Gays,Travestis,Swingers,Energizantes,Erotico,Porno,EntreAngeles.site,Sexo Facil,Mujeres Calientes,Juguetes Sexuales,Scorts">
		<link rel="icon" href="<?=DOMINIO?>Design/assets/images/icon.ico" type="image/x-icon"/>
		<link rel="shortcut icon" type="image/x-icon" h	ref="<?=DOMINIO?>Design/assets/images/icon.ico" />
					...
					<div class="horizontal-mainwrapper container clearfix">
						...
						<nav class="horizontalMenu clearfix d-md-flex">
							<ul class="horizontalMenu-list">
								...
								<li aria-haspopup="true"><a href="about.html">Acerca de Nosotros</a></li>
